...
function getReviews() {
    ...
    currentProduct.reviews.forEach((e, index) => {
        ...
        for (let i = 1; i <= 5; i++) {
            if (i <= e.rating) {
                rate.insertAdjacentHTML("beforeend", '<img src="./assets-images/gold_star.svg" alt="star">')
            }
            else if (i > e.rating) {
                rate.insertAdjacentHTML("beforeend", '<img src="./assets-images/silver_star.svg" alt="star">')
            }
        }
    })
}
